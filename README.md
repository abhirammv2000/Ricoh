...
| 12 | Measure the prompt-injection guardrail, don't just unit-test it | **Done.** `eval/redteam_guardrail.py` runs 40 adversarial prompts across 4 categories: the regex blocks 10/10 of what it was written for, 1/10 obfuscated variants, 0/12 novel techniques, and critically 0/8 false positives. The `--full-pipeline` judged run then checked whether grounding holds on the 29 the regex missed: 28/29 held; one produced a soft, paraphrased leak of the synthesizer's role (not verbatim, and the refusal itself was still correct). [§7](#7-evaluation-and-metrics). |
