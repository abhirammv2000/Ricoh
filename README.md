...
### Measured results (real run, not claims)

Full corpus (733 PDFs), Claude Sonnet 4.6 as both the agent LLM and the judge, 10 official questions. Generated by `src/eval_harness.py` → [eval/eval_report.md](eval/eval_report.md) / [eval/metrics.json](eval/metrics.json):

| Metric | Score | Meaning |
|---|---|---|
| **Behaviour-match rate** | **100%** | Answered when answerable, refused when not — all 10 correct |
| **Mean groundedness** | **0.98** | Claims supported by retrieved evidence (LLM-judged) — near-zero hallucination |
| **Mean answer correctness** | **0.97** | Conveys the expected key facts / refuses correctly (LLM-judged) |
| **Mean citation precision** | **1.00** | Every cited document is real — **zero fabricated citations** across all questions |
| **Mean retrieval recall@k** | **0.78** | Expected source doc retrieved (the weakest metric — see caveats) |
| **Mean latency** | **17.4s** | Per question, end-to-end (planner + 2-pass retrieval + verify + synthesize) |

**Honest caveats on these numbers:**
- **Small sample, single judge.** 10 questions scored by one LLM judge (Sonnet). This is a credible signal, not a statistically rigorous benchmark. Groundedness/correctness are model-judged, not human-graded.
